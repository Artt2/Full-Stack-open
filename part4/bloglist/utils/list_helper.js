...
const mostBlogs = (blogs) => {
  ...
  const mostBlog = _.maxBy(
    _.keys(authorsGrouped),
    (author) => authorsGrouped[author].length
  );
  return {
    author: mostBlog,
...
const mostLikes = (blogs) => {
  ...
  const mostLiked = _.maxBy(_.keys(authorsGrouped), (author) => {
    const likes = authorsGrouped[author].reduce(
  ...
  const mostLikes = authorsGrouped[mostLiked].reduce(
    (accumulator, current) => accumulator + current.likes,
    0
  );
